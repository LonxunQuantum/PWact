...
class Image():
    ...
    """
    @Description :
        fread_dfeat/feat.info
        fread_dfeat/fit_linearMM.input
        fread_dfeat/linear_fitB.ntype
    @Returns     :
    @Author       :wuxingxing
    """
    def set_fread_dfeat(self, fread_dfeat):
        self.feat_info = os.path.join(fread_dfeat, "feat.info")
        self.fit_linearMM_input =  os.path.join(fread_dfeat, "fit_linearMM.input")
        self.linear_fitB_ntype =  os.path.join(fread_dfeat, "linear_fitB.ntype")

    def calc_egroup(self):
        # to line 40, get e_atom0
        f = open(self.feat_info, 'r')
        txt = f.readlines()
        f.close()
        ...
        f = open(self.fit_linearMM_input, 'r')
        txt = f.readlines()
        f.close()
        ...
        f = open(self.linear_fitB_ntype, 'r')
        txt = f.readlines()
        f.close()
...
def read_dft_movement(file_dft, fread_dfeat):
    f = open(file_dft, 'r')
    txt = f.readlines()
    f.close()
    ...
    for i in range(num_iters):
        ...
        image.set_fread_dfeat(fread_dfeat)
        image.calc_egroup()
...
if __name__ == '__main__':
    file_dft = "/home/wuxingxing/datas/system_config/cu_4phases_system/dpkf_data/slab/dpkf_saperate/1300k/200/PWdata/MOVEMENT"
    file_dpkf = "/home/wuxingxing/datas/active_learning_dir/cu_slab_1300k_system/bkfiles/slab_1300k_all_dpkfmd_kpu/md_dir/MOVEMENT"
    pic_save_path = "/home/wuxingxing/datas/active_learning_dir/cu_slab_1300k_system/draw_dir"
    fread_dfeat = "/home/wuxingxing/datas/active_learning_dir/cu_slab_1300k_system/init_data/fread_dfeat"
    num_atoms, num_iters, dft_atomic_energy, dft_force, dft_group_energy = read_dft_movement(file_dft, fread_dfeat)
    tmp_natom, tmp_niters, nn_atomic_energy, nn_force, nn_group_energy = read_dft_movement(file_dpkf, fread_dfeat)
    
    #nn_atomic_energy, nn_force, nn_group_energy = read_nn_movement()
    #print(dft_group_energy[0:100])
    from sklearn.metrics import mean_squared_error, r2_score
    #np.sqrt(mean_squared_error(result, test_y))
    for i in range(0, len(dft_atomic_energy) - 72, 72):
        print("image {} etot_rmse: {} dft_etot: {} pre_etot: {} force_rmse{}".format(
                i/72,
                np.sqrt(mean_squared_error(dft_atomic_energy[i:i+72], nn_atomic_energy[i:i+72])), \
                    sum(dft_atomic_energy[i:i+72]), sum(nn_atomic_energy[i:i+72]),\
                        np.sqrt(mean_squared_error(dft_force[i:i+72], nn_force[i:i+72]))))

    lens = 30
    num_iters = lens
    tmp_niters = lens
    dft_atomic_energy = dft_atomic_energy[:lens*num_atoms]
    dft_force = dft_force[:lens*num_atoms]
    dft_group_energy = dft_group_energy[:lens*num_atoms]

    nn_atomic_energy = nn_atomic_energy[:lens*num_atoms]
    nn_force = nn_force[:lens*num_atoms]
    nn_group_energy = nn_group_energy[:lens*num_atoms]
    ...
    # plot atomic energy
    plt.subplot(2,2,1)
    plt.title('atomic energy',fontsize=10)
    plt.scatter(dft_atomic_energy, nn_atomic_energy, s=0.1)
    plt.plot((lim_min, lim_max), (lim_min, lim_max), ls='--', color='red')
    plt.xlim(lim_min, lim_max)
    plt.ylim(lim_min, lim_max)
    plt.xlabel('DFT atomic energy (eV)',fontsize = 8)
    plt.ylabel('MLFF atomic energy (eV)',fontsize = 8)
    ...
    plt.xlabel('DFT group energy (eV)',fontsize=8)
    ...
    plt.text(e_tot_min, e_tot_max,'rmse: %.3E' %(e_tot_rms),fontsize = 8)
    ...
    plt.text(fmin, fmax,'rmse: %.3E' %(f_rms),fontsize = 8)
    ...
    plt.savefig(os.path.join(pic_save_path, 'DFT_dpkf_MD_1000_30md.png'), format='png')
    #if len(sys.argv) < 2:
    #    plt.show()
